...
def load_data():
    with open(f"{DATA_DIR}/orders.csv", 'r') as f:
        orders = {row[0] for row in csv.reader(f, delimiter='|') if row}
    data = []
    with open(f"{DATA_DIR}/lineitem.csv", 'r') as f:
        for row in csv.reader(f, delimiter='|'):
            if row and row[0] in orders:
                data.append(row[0])
    return data
...
def main():
    ...
    with open("q12_results.csv", "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["GSQ", "avg_error", "std_dev"])
        writer.writeheader()
        writer.writerows(results)

    print("\n结果已保存到 q12_results.csv")
